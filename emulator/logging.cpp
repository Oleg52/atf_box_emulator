...
const char* TRANSMIT_DUMP_FILE_NAME = "C:\\transmit_dump_info.txt";
...
{
#ifdef ENABLE_LOGS
    FILE* f = fopen(TRANSMIT_DUMP_FILE_NAME, "a");
    if (!f) return;
	...
	int bytesWritten = 0;
	unsigned int i = 0;
	for (; i < sendBufferLength; i++)
	{
		fprintf(f, "0x%02X", sendBuffer[i]);
		if (i != sendBufferLength - 1) fprintf(f, ",");
	}

	fprintf(f, "}, {");

	i = 0;
	for (; i < recvBufferLength; i++)
	{
		fprintf(f, "0x%02X", recvBuffer[i]);
		if (i != recvBufferLength - 1) fprintf(f, ",");
	}
